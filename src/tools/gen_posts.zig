...
pub fn main() !void {
    ...
    var posts = std.ArrayList(u8).init(allocator);
    var postjsons = std.ArrayList(PostJson).init(allocator);
    ...
    while (try iter.next()) |entry| {
        if (entry.kind == .directory) {
            ...
            const pj_clone = PostJson{
                .name = try allocator.dupe(u8, pj_parse.value.name),
                .id = try allocator.dupe(u8, pj_parse.value.id),
                .date = try allocator.dupe(u8, pj_parse.value.date),
